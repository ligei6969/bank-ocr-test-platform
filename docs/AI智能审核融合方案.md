...
## 十、P1 完成实况与下一步路线（含「银行业务知识客服 Agent」方向）

> 本节记录 P1 的**实际交付与核验结论**，并给出到 P2/P3/P4 的修订路线。
> 与原计划的差异：在原路线之间**插入一条新主线 —— 银行业务知识客服 Agent**，
> 它是「AI 应用 + Agent 测试」两个岗位叙事的最佳延伸，且复用 P1 内核、不动主链路。
> 交付报告见 `docs/P1_Agent开发报告.md`；下一步执行计划见 `docs/下一步开发计划.md`。

### 10.1 P1 交付物（已核验）

| 位置 | 内容 |
| --- | --- |
| `ai_service/agent.py` | planner + executor 多步决策循环；三重预算（步数 / token / 工具调用）；白名单与参数校验；两条路径同结构 |
| `ai_service/tools.py` | 白名单四工具：`search_knowledge` / `get_review_record` / `recompute_quality` / `escalate_to_human`，全部只读无副作用 |
| `ai_service/thresholds.py` | 质检阈值的机器可读副本 + 一致性测试（从平台源码抽真值比对，防漂移） |
| `ai_service/prompts.py` | Prompt 版本化注册表（id + version + label，进入 trace 与响应） |
| `ai_service/structured.py` | 结构化输出解析 + pydantic 校验 |
| `ai_service/cassette.py` | LLM / 工具的 record-replay；replay 未命中抛 `BaseException` 子类，杜绝静默降级 |
| `ai_service/agentkit.py` | 轨迹断言、故障注入、cassette 装配 |
| `ai_service/eval/` | golden 集、四层指标、judge 与校准、报告编排、baseline |
| `scripts/evaluate_ai_review.py` | 评测 CLI，退化超阈值返回非零码，接 CI |
| `.github/workflows/tests.yml` | 固定 `LLM_PROVIDER=none`；新增评测门禁与 Allure 上传 |

**测试规模**：平台侧 372 → 387，AI 服务侧 75 → 260，合计 **447 → 647**，全绿。
**主链路零改动**：`app/main.py` 的 `/bank-card/review`、`/id-card/review` 一行未动。

### 10.2 P1 遗留缺口（诚实清单）

这些不是失败，是**下一步的输入**。处理它们本身就是面试里「你知道自己系统的边界在哪」的证明。

| 缺口 | 现状 | 影响的岗位叙事 | 处理阶段 |
| --- | --- | --- | --- |
| **真实模型端到端未验证** | 本机无 LLM key，`--live` 只覆盖了「无 key 友好提示」分支；cassette 的 live 录制用假模型测过 | AI 应用岗必问「你真调过模型吗」 | **P1.5（下一步第一件事）** |
| **决策层无真实结论标注** | `labels.json` 只标字段真值与退化类型，任务层用「原因码命中率」代理 | 测试岗「怎么衡量结论对错」 | P2 前置 |
| **judge 校准集是占位** | 10 条为作者自评，仅验证管线、无统计意义 | 测试岗「judge 怎么校准」 | P2 |
| **成本为字符估算** | `LLMClient` 协议拿不到 provider usage | 应用岗「成本怎么控」 | P1.5 可顺带做 |
| **Agent 未接管理端 UI** | `/agent/explain` 已可用，前端仍渲染 P0 结构 | 展示完整度 | P3 |

### 10.3 新主线：银行业务知识客服 Agent

**为什么值得单开一条主线**

现有 Agent 是「审核员解释器」——服务对象是审核员，回答「这条记录为什么是这个结论」。
这证明了你**会做 Agent**，但它只有一个业务面。而「AI 应用开发」和「Agent 测试开发」
两个岗位真正想看的是：**同一套内核能不能支撑不同产品面**。

银行业务知识客服恰好是这样一个产品面，而且有三个不可替代的价值：

1. **复用而非重写**：检索链路、工具框架、cassette、轨迹断言、评测管线全部复用，
   新增的只是一个 corpus + 一套工具 + 一个 persona。这展示的是**平台思维**，不是功能堆叠。
2. **测试维度天然不同**：审核域测的是「解释准不准」，客服域测的是
   **幻觉与接地、越界拒答、合规话术** —— 这是三个审核域测不了的新维度，直接命中
   Agent 测试岗的能力清单。
3. **风险更低**：客服 Agent 独立于审核主链路，做错了不影响任何既有自动化。

**定位：先做对内，再谈对外**

本方案主张 **P2.1 先做「对内知识客服」**（服务对象是平台用户 / 客户经理，
查业务规则、材料清单、常见问题），**P2.2 再评估对外**（面向真实用户）。
理由与 10.4 的安全边界直接相关：对内可以放开业务知识深度，对外必须收紧到
「不泄露审核内部信息、不给个性化金融建议」的严格边界。先对内跑通，
把拒答与合规测明白，再对外，风险可控。

**架构（复用 P1 内核）**

```
                         ai_service/  (独立进程 :8100)
   ┌───────────────────────────┬───────────────────────────────┐
   │  surface: 审核解释 Agent    │  surface: 知识客服 Agent（新）   │
   │  corpus: 审核原因码/规范     │  corpus: 银行业务知识/FAQ/话术    │
   │  tools: 4 个审核工具         │  tools: 知识检索/产品查询/FAQ/转人工│
   └───────────────┬───────────┴───────────────┬───────────────┘
                   │      共用内核（不复制）      │
        ┌──────────▼──────────┐   ┌─────────────▼─────────────┐
        │ agent.py 决策循环    │   │ cassette.py 录制回放       │
        │ tool_manager.py 工具 │   │ agentkit.py 轨迹断言/注入  │
        │ prompts.py 版本化    │   │ eval/ 指标与门禁           │
        └─────────────────────┘   └───────────────────────────┘
```
...
| 工具 | 职责 | 降级 |
| --- | --- | --- |
| `search_faq` | 检索业务 FAQ、材料清单、办理流程 | 退回关键词直查 |
| `search_policy` | 检索行内业务规则/合规口径 | 同上 |
| `lookup_product` | 查询产品参数（卡种、费率口径）—— **只读固定语料，不接实时系统** | 明确「暂无法查询」 |
| `handoff_to_human` | 转人工/工单 | 给出工单指引 |

**与审核 Agent 的关键差异**

- persona 不同：客服是「帮用户/客户经理搞懂业务」，不是「复核一条记录」。
- corpus 不同：业务知识文档规模会比审核域大（几十 → 上千片段），
  届时可能需要评估是否引入向量后端 —— 但**依然先做词法 + 哈希混合**，
  语料没到量就不引入重依赖（保持 P0 立场一致性）。
- **安全边界更硬**（见 10.4）。

### 10.4 安全边界（客服 Agent 的硬约束）

这是本主线最有价值、也最容易翻车的地方。以下每条都必须有对应测试：

1. **不泄露审核内部信息**。客服 Agent 不得输出审核原因码、OCR 原文、
   完整证件号、内部规则阈值。用户问「我的审核为什么被拒」→ 引导到正确的业务渠道，
   而不是复述内部原因码。
2. **不给个性化金融建议**。额度、利率、能否批卡、是否该办某产品一律拒答，
   改为提供**通用产品信息**并建议咨询人工。
3. **拒绝越权与猜测**。答不出就说「暂无法确认，建议咨询人工」，
   **不得编造业务规则**（幻觉是客服域的头号风险）。
4. **合规话术**。涉及个人信息、资金安全的问题按行内合规口径回应。
5. **不绕开脱敏**。客服入参同样经过 `sanitize_review_fields` / `sanitize_for_log`。

### 10.5 客服 Agent 的测试体系（三个新维度）

在 P1 已有的「轨迹断言 / cassette 回放 / 故障注入 / 对抗输入」之上，新增：

| 新维度 | 测什么 | 手法 |
| --- | --- | --- |
| **接地 / 幻觉** | 每个事实性断言必须能追溯到检索到的知识片段 | 答案中的数字/规则与 citations 做交叉校验；无来源的事实一律判失败 |
| **越界拒答** | 问到不该答的（额度/利率/审核内部）能否礼貌拒绝 | 预置越界问题集 + 期望「拒答 + 引导」，而不是「硬答」 |
| **合规话术** | 涉及个人信息/资金安全时是否命中合规口径 | 固定话术模板 + judge 或规则校验关键短语 |

这三类都进 golden 集与回归门禁，指标同样分「工具层 / 任务层 / 解释层 / 回归层」。

### 10.6 修订后的总路线图

| 阶段 | 内容 | 岗位叙事 | 依赖 |
| --- | --- | --- | --- |
| **P1.5** | 补 P1 尾巴：真实模型端到端 + usage 成本 + （可选）结论标注 | 收口，消除「未验证」项 | 需 LLM key |
| **P2.1** | 对内银行业务知识客服 Agent（复用内核 + 新 corpus/工具/边界） | AI 应用：一内核多产品面 | P1 内核 |
| **P2.2** | 对外客服评估（视 P2.1 效果） | AI 应用：产品化与合规 | P2.1 |
| **P2.3** | 规则 + LLM 双判 + `llm_override` 落库 | AI 应用：闭环可评估 | 结论标注 |
| **P3** | 监控：`/metrics`、AI 改判率、回滚率；Agent trace 接入管理端 UI | 两岗通用：可观测 | P1/P2 |
| **P4** | 多轮会话记忆 + MCP server（把审核/客服能力暴露为 MCP 工具） | AI 应用：生态与协议 | P2 |

> 路线原则：**P1.5 先收口**（把「未验证」清零），**P2.1 立新面**（客服 Agent），
> **P2.3 补闭环**（双判落库），再谈观测与生态。每一步都保持「主链路零侵入 + 全离线可测」。

### 10.7 面试讲法（加入客服 Agent 后）

**AI 应用开发岗** —— 主线用 P1.5 + P2.1 + P2.3：

- 同一套 Agent 内核如何支撑「审核解释」与「知识客服」两个产品面（平台思维）
- 客服 Agent 的安全边界怎么设计：不做个性化金融建议、不泄露审核内部信息
- 幻觉怎么治：事实必须可追溯到知识片段，而不是靠 prompt 说「别编」
- `llm_override` 落库为什么是评估的前提

**Agent 测试开发岗** —— 主线用 P1.3 + P1.4 + P2.1 的测试体系：

- 不确定系统怎么在 CI 里确定（cassette 回放，未命中即失败）
- 怎么断言轨迹而非文本；怎么注入工具故障；怎么测越权与 injection
- 客服域的**接地 / 越界 / 合规**三关怎么自动化
- judge 怎么校准；任务成功率 / 步数 / 成本怎么做成回归门禁
